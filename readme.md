...
* **Interactive Paper Tape:** Expression visualization in textbook format (vertical fractions, symbolic indefinite integrals, definite integrals with limits, analytical derivatives, limits, and matrix notation).
...
* **Extended Mathematical Engine (CAS / `algebrite` + `mathjs`):**
  * **Computer Algebra System (CAS):** Symbolic integration (`integrate`), algebraic expansion (`expandExpr`), and full symbolic factorization (`factor`).
  ...
  * **Infinitesimal Calculus:** Symbolic derivatives, numerical definite integrals (Simpson's rule), and lateral limits.
...
| **Factoring** | `factor('x^4 - 16')` | Full CAS symbolic expression factorization |
| **Expansion** | `expandExpr('(x + 2)^3')` | Algebraic expansion using CAS |
...
| **Calculus** | `integrate('x * sin(x)', 'x')`| Symbolic Indefinite Integration |
